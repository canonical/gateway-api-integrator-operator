...
# A pinned reference is a semantic version tag, a tf-X.Y.Z tag, or a git commit SHA.
...
def _unquote(value: str) -> str:
    ...
    Args:
        value: A raw string value or label from a parsed Terraform file.

    Returns:
        The value without its surrounding double quotes.
    """
...
def block_names(parsed: dict, block_type: str) -> list[str]:
    ...
    Args:
        parsed: A python-hcl2 parsed Terraform file.
        block_type: The block keyword, for example "variable" or "output".

    Returns:
        The labels in the order they appear in the file.
    """
    names: list[str] = []
...
def check_terraform_block(parsed: dict) -> list[str]:
    ...
    Args:
        parsed: A python-hcl2 parsed terraform.tf.
...
def check_alphabetical(parsed: dict, block_type: str, filename: str) -> list[str]:
    ...
    Args:
        parsed: A python-hcl2 parsed Terraform file.
        block_type: The block keyword, for example "variable".
        filename: Name of the file, used in the violation message.

    Returns:
        A list with a single message when unordered, otherwise empty.
    """
...
def is_product_module(parsed_files: list[dict]) -> bool:
    ...
    Args:
        parsed_files: The parsed contents of the module's Terraform files.

    Returns:
        True if at least one module block is present.
    """
...
def check_interface(variables: list[str], outputs: list[str], product: bool) -> list[str]:
    ...
    Args:
        variables: Variable names declared by the module.
        outputs: Output names declared by the module.
        product: Whether the module is a product/composed module.
...
def check_pinned_module_sources(parsed_files: list[dict]) -> list[str]:
    ...
    Local sources (./ or ../) and registry sources with a version are allowed.
...
def _load(path: Path) -> dict:
    ...
    Args:
        path: Path to the Terraform file.
...
def main(argv: list[str] | None = None) -> int:
    ...
    Returns:
        Process exit code: 0 when all modules are compliant, 1 otherwise.
    """
