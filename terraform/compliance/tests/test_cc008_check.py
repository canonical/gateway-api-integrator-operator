...
def test_compliant_terraform_block_has_no_violations() -> None:
    parsed = hcl2.loads(COMPLIANT_TERRAFORM_TF)

    assert cc008_check.check_terraform_block(parsed) == []
...
def test_repository_modules_are_compliant() -> None:
    charm = REPO_ROOT / "terraform" / "modules" / "gateway-api-integrator"
    product = REPO_ROOT / "terraform" / "product"

    assert cc008_check.check_module(charm) == []
    assert cc008_check.check_module(product) == []
